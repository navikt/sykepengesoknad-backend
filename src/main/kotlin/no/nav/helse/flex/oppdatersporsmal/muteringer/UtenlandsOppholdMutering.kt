...
fun Sykepengesoknad.oppdaterMedSvarPaUtlandsopphold(): Sykepengesoknad {
    ...
    val maybeSoktOmSykepengerSporsmal = getOptionalSporsmalMedTag(UTLANDSOPPHOLD_SOKT_SYKEPENGER)
...
private fun Sykepengesoknad.hentGyldigePerioder(hva: String, nar: String): List<Periode> {
    return if (this.harFeriePermisjonEllerUtenlandsoppholdSporsmal()) {
        val harFeriePermisjonUtlandsopphold =
            this.getSporsmalMedTag(FERIE_PERMISJON_UTLAND).forsteSvar == "JA"
        if (harFeriePermisjonUtlandsopphold && this.getSporsmalMedTag(hva).forsteSvar == "CHECKED")
            getGyldigePeriodesvar(this.getSporsmalMedTag(nar))
        else
            Collections.emptyList()
    } else {
        val harSvartJa = this.getSporsmalMedTag(hva).forsteSvar == "JA"
        if (harSvartJa)
            getGyldigePeriodesvar(this.getSporsmalMedTag(nar))
        else
            Collections.emptyList()
