...
class OverlappMetrikkJob(
    ...
    fun job() {
        if (leaderElection.isLeader()) {
            ...
            duplikater.forEach {
                require(it.variant.startsWith("SOKNAD")) {
                    "Fjerner bare duplikate varianter som starter med SOKNAD, ${it.variant}"
                }

                klippMetrikkRepository.fjernDuplikat(
                    it.sykmeldingUuid,
                    it.eksisterendeSykepengesoknadId,
                    it.variant
                )
            }

            require(klippMetrikkRepository.findDuplikateMetrikker().isEmpty()) {
                "Det finnes fremdeles duplikater etter sletting"
            }
        }
    }
}
