...
class FriskTilArbeidService(
    ...
    fun schedulertStartBehandlingAvFriskTilArbeidVedtakStatus() {
        if (leaderElection.isLeader()) {
            val vedtak = friskTilArbeidRepository.findById("f9edec11-d550-476e-a95b-67c31824242c")
            if (vedtak.isPresent) {
                val oppdatertVedtak = vedtak.get().copy(fom = LocalDate.of(2025, 9, 6))
                friskTilArbeidRepository.save(oppdatertVedtak)
                log.info(
                    "Oppdaterte FriskTilArbeid-vedta med id: ${oppdatertVedtak.id} fra fom: ${vedtak.get().fom} til fom: ${oppdatertVedtak.fom}.",
                )
            } else {
                log.info("Fant ikke FriskTilArbeid-vedtak med id: f9edec11-d550-476e-a95b-67c31824242c.")
            }
        }
    }
