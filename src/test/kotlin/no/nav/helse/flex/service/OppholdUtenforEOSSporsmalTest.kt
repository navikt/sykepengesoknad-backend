...
class OppholdUtenforEOSSporsmalTest : FellesTestOppsett() {
    ...
    @Test
    fun `fjerner det nye opphold utland spørsmålet som ble lagt til ved en feil`() {
        mockFlexSyketilfelleArbeidsgiverperiode()
        val opprettetSoknader =
            sendSykmelding(
                sykmeldingKafkaMessage(
                    fnr = fnr,
                    sykmeldingsperioder = heltSykmeldt(fom = fom, tom = tom),
        ...
        val soknadMedDobbelUtlandSporsmal =
            sykepengesoknadDAO.finnSykepengesoknad(opprettetSoknader.first().id)
                .leggTilSporsmaal(oppholdUtenforEOSSporsmal(fom, tom))
        sykepengesoknadDAO.lagreSykepengesoknad(soknadMedDobbelUtlandSporsmal)
        ...
        hentetSoknad.sporsmal!!.map { it.tag } `should be equal to`
            listOf(
                ANSVARSERKLARING,
                TILBAKE_I_ARBEID,
                FERIE_V2,
