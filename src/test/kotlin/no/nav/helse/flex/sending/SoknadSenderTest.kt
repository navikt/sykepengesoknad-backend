...
class SoknadSenderTest : FellesTestOppsett() {
    ...
    private fun sjekkSoknad(sendtSoknad: RSSykepengesoknad) {
    ...
    fun `lager ikke søknad hvis utlandsopphold er i helg`() {
        fellesOppsett()
        val soknaden = hentSoknad(soknadId = hentSoknaderMetadata(fnr).first { it.status == RSSoknadstatus.NY }.id, fnr = fnr)
    ...
    @Test
    fun `lager ikke søknad hvis utlandsopphold er i ferie`() {
        fellesOppsett()
        val soknaden = hentSoknad(soknadId = hentSoknaderMetadata(fnr).first { it.status == RSSoknadstatus.NY }.id, fnr = fnr)
        ...
        val sendtSoknad =
            SoknadBesvarer(rSSykepengesoknad = soknaden, mockMvc = this, fnr = fnr)
                ...
                .besvarSporsmal(
                    OPPHOLD_UTENFOR_EOS_NAR,
                    svar = """{"fom":"${soknaden.fom!!.plusDays(3)}","tom":"${soknaden.fom!!.plusDays(5)}"}""",
                    mutert = false,
                    ferdigBesvart = true,
                )
                .besvarSporsmal(tag = "ARBEID_UNDERVEIS_100_PROSENT_0", svar = "NEI")
