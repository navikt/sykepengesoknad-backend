...
internal class SvarValideringTest {
    ...
    fun `test validering av beløp spørsmål`() {
        ...
        spm.byttSvar(svar = "1.0") `valider svar og forvent feilmelding`
            "Spørsmål ${spm.id} med tag OFFENTLIG_TRANSPORT_BELOP har feil svarverdi 1.0"
        spm.byttSvar(svar = "-1") `valider svar og forvent feilmelding`
            "Spørsmål ${spm.id} med tag OFFENTLIG_TRANSPORT_BELOP har svarverdi utenfor grenseverdi -1"
    ...
    fun `test validering av kvittering`() {
        ...
        spm.byttSvar(svar = "ikke jsånn") `valider svar og forvent feilmelding`
            "Spørsmål ${spm.id} med tag KVITTERINGER har feil svarverdi ikke jsånn"
        val kvittering =
            Kvittering(
                belop = 234,
        ...
        // krever uuid
        spm.byttSvar(svar = kvittering.copy(blobId = "ikkeuuid").serialisertTilString()) `valider svar og forvent feilmelding`
            "Spørsmål ${spm.id} med tag KVITTERINGER har feil svarverdi"

        // Krever positivt beløp
        spm.byttSvar(svar = kvittering.copy(belop = -20).serialisertTilString()) `valider svar og forvent feilmelding`
            "Spørsmål ${spm.id} med tag KVITTERINGER har feil svarverdi"

        // Feiler når råteksten er for lang for databasen
        spm.byttSvar(svar = kvittering.serialisertTilString().padEnd(1000)) `valider svar og forvent feilmelding`
            "Spørsmål ${spm.id} med tag KVITTERINGER har svarverdi utenfor grenseverdi"
    }
    ...
    fun `test reiseMedBilSpørsmål`() {
        ...
        spm.byttSvar(svar = "KANSKJE") `valider svar og forvent feilmelding`
            "Spørsmål ${spm.id} med tag $REISE_MED_BIL har feil svarverdi KANSKJE"

        spm.byttSvar(svar = "JA") `valider svar og forvent feilmelding`
            "Spørsmål ${spm.idForTag(BIL_DATOER)} med tag $BIL_DATOER har feil antall svar 0"
    ...
    fun `test opphold utenfor EØS spørsmål`() {
        ...
        spm.byttSvar(svar = "JA") `valider svar og forvent feilmelding`
            "Spørsmål ${spm.id} med tag $OPPHOLD_UTENFOR_EOS_NAR har feil antall svar 0"

        spm
            .byttSvar(tag = OPPHOLD_UTENFOR_EOS, svar = "JA")
    ...
    private fun Sporsmal.idForTag(tag: String): String? = listOf(this).flatten().first { it.tag == tag }.id
