...
class GradertReisetilskuddIntegrationTest : FellesTestOppsett() {
    ...
    fun hentAlleKafkaMeldinger() {
        juridiskVurderingKafkaConsumer.ventPåRecords(5)
    }
    ...
    fun `Søknaden har alle spørsmål før vi har svart på om reisetilskuddet ble brukt`() {
        ...
        assertThat(
            soknaden.getSporsmalMedTag(TILBAKE_I_ARBEID).sporsmalstekst,
        ).isEqualTo("Var du tilbake i ditt vanlige arbeid uten ekstra reiseutgifter før 7. september?")
        assertThat(
            soknaden.getSporsmalMedTag("JOBBET_DU_GRADERT_0").sporsmalstekst,
        ).isEqualTo(
    ...
    fun `Vi ombestemmer oss å svarer nei på reisetilskudd brukt`() {
        ...
        val utkastMedSvar =
            SoknadBesvarer(utkast, this, fnr)
